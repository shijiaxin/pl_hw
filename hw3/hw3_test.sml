...
typecheck_patterns([], [TupleP [Variable "x", Variable "y"],Wildcard]);
typecheck_patterns([], [TupleP [ConstP 10, Wildcard],TupleP [Wildcard,TupleP[Wildcard, UnitP]]]);
typecheck_patterns([], [TupleP [Wildcard, TupleP [Wildcard, Wildcard]],TupleP [Wildcard, Wildcard]]);
typecheck_patterns([], [ConstructorP("c", Variable "x")]);
typecheck_patterns([("c1", "t1", TupleT[IntT, UnitT]),("c2", "t2", UnitT) ]
                    ,[ConstructorP("c1", TupleP [Wildcard, Variable "x"])]);
typecheck_patterns([("c1", "t1", TupleT[IntT, UnitT]),("c2", "t2", TupleT[IntT,Datatype "t1"])]
                    ,[ConstructorP("c2", TupleP [Wildcard,ConstructorP("c1",TupleP[Wildcard, UnitP])])]);
typecheck_patterns([("c1", "t1", TupleT[IntT, UnitT]),("c2", "t2", TupleT[IntT,Datatype "t1"])]
                    ,[ConstructorP("c2", TupleP [Wildcard,ConstructorP("c1",TupleP[UnitP, Wildcard])])]);
(*
SOME (TupleT [Anything,Anything]) : typ option
SOME (TupleT [Anything,Anything]) : typ option
SOME (TupleT [IntT,TupleT [#,#]]) : typ option
SOME (TupleT [Anything,TupleT [#,#]]) : typ option
NONE : typ option
SOME (Datatype "t1") : typ option
SOME (Datatype "t2") : typ option
NONE : typ option
*)
